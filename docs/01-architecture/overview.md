...
| Integration | 8021 | External integrations, API management, API marketplace, connector framework, EDI, MDM, data governance, trade compliance, event mesh, blockchain integration, enterprise data quality |
...
> **Note:** Service-to-service authentication (mTLS + Service JWT) is a security mechanism that applies to all communication patterns — it is not a separate communication pattern. See [Security](../02-security/overview.md) for details.

*See [Services](../06-services/overview.md) for detailed per-service specifications.*
