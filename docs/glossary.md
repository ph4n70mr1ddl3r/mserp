...
| Blue-Green Deployment | Deployment strategy maintaining two identical production environments (blue and green), switching traffic between them for zero-downtime releases and instant rollback capability |
...
| Brute-Force Detection | Security mechanism that identifies and blocks repeated failed authentication attempts from a single source, using rate limiting, progressive delays, and account lockout to prevent credential guessing |
...
| Data Lake | Centralized repository that stores structured and unstructured data at any scale in raw form (MinIO bronze/silver/gold zones in MSERP), enabling batch and real-time analytics |
...
| Event Sourcing | Pattern where state changes are stored as an immutable sequence of events rather than overwriting current state, enabling complete audit trails, temporal queries, and event replay |
| EVM | Earned Value Management — project management technique for measuring performance |
| Feature Flags | Configuration-driven toggles that enable or disable application features at runtime without code deployment, supporting progressive rollouts, A/B testing, and tenant-specific feature control |
...
| Session Management | Handling of user authentication sessions including creation, validation, renewal, and revocation — in MSERP implemented via Redis-backed sessions with sliding expiration and concurrent session limits |
...
| Step-Up Authentication | Security mechanism requiring re-authentication or stronger authentication (e.g., MFA) for high-risk operations, even if the user already has an active session |
...
| Tenant | Logical isolation boundary in a multi-tenant SaaS system — each tenant has its own data partition (enforced via PostgreSQL RLS), configuration, user directory, and feature entitlements |
