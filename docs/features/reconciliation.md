...
|-------|---------|---------|-----------|
| `reconciliation.match.completed` | `{reconciliation_id, account_id, matched_count, unmatched_count}` | Auto-match run completes | Close Task Manager, Dashboard |
| `reconciliation.exception.created` | `{exception_id, reconciliation_id, transaction_ids, reason}` | Unmatched or low-confidence match detected | Workflow Engine, Notification Service |
...
| `reconciliation.close.certified` | `{period_id, account_id, certified_by, certified_at}` | User certifies reconciliation complete | Close Task Manager, Report Service |
...
- [NFR: Performance](../planning/nfr.md) — Reconciliation auto-match < 30s per account
