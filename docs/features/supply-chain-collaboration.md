...
|-------|---------|---------|-----------|
| `scc.forecast.published` | `{forecast_id, supplier_id, period_range, granularity, items[], commitment_level}` | Demand forecast shared with supplier | Supplier Portal, Notification |
...
| `scc.capacity.committed` | `{commitment_id, supplier_id, buyer_id, category, qty, period, status}` | Buyer commits to supplier capacity | Supplier Portal, Planning |
...
| `scc.asn.submitted` | `{asn_id, supplier_id, po_id, ship_date, carrier, lines[], tracking}` | Supplier submits ASN | Receiving Warehouse, AP |
...
- [Commerce Service](../services/commerce.md) — Order management and supplier portal
- [Integration Service](../services/integration.md) — Supplier network connectors and EDI
